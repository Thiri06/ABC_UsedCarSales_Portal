...
<body>
    ...
    <div class="container-fluid">
        <div class="row justify-content-around mt-5 mb-5">
            ...
            <div class="col-md-9 mb-5">
                <h4 class="mb-4 title-bar">Your Appointments</h4>
                ...
                <div class="dashboard-container">
                    ...
                    @if ($appointments->isEmpty())
                        <div class="text-center mt-4">
                            <p class="text-muted"><i class="fas fa-calendar-times"></i> No appointment requests yet.</p>
                        </div>
                    @else
                        @foreach ($appointments as $appointment)
                            <div class="card shadow-sm mb-4">
                                <div class="card-header bg-light d-flex justify-content-between align-items-center">
                                    <h5 class="mb-0">Appointment Request</h5>
                                    <span class="badge bg-{{ $appointment->status === 'accepted' ? 'success' : ($appointment->status === 'rejected' ? 'danger' : 'secondary') }}">
                                        {{ ucfirst($appointment->status) }}
                                    </span>
                                </div>
                                <div class="card-body">
                                    <p><i class="fas fa-car"></i> <strong>Car:</strong> {{ $appointment->carPost->make }} {{ $appointment->carPost->model }}</p>
                                    <p><i class="fas fa-user"></i> <strong>Requested By:</strong> {{ $appointment->user->name }}</p>
                                    <p><i class="fas fa-calendar"></i> <strong>Date:</strong> {{ \Carbon\Carbon::parse($appointment->appointment_date)->format('d M, Y') }}</p>
                                    <p><i class="fas fa-clock"></i> <strong>Time:</strong> {{ \Carbon\Carbon::parse($appointment->appointment_time)->format('h:i A') }}</p>
                                    <p class="text-muted small">
                                        <i class="fas fa-clock"></i> Requested: {{ $appointment->created_at->format('d M, Y h:i A') }}
                                    </p>

                                    <div class="d-flex">
                                        @if ($appointment->status === 'pending')
                                            <form method="POST" action="{{ route('user.handleAppointment', $appointment->id) }}" class="me-2">
                                                @csrf
                                                @method('PUT')
                                        ...
                                        @endif
                                        <form method="POST" action="{{ route('user.deleteAppointment', $appointment->id) }}" onsubmit="return confirm('Are you sure you want to delete this appointment?');">
                                            @csrf
                                            @method('DELETE')
    ...
    <script>
        function dismissNotification(notificationId) {
            // Send AJAX request to mark the notification as read or delete it
